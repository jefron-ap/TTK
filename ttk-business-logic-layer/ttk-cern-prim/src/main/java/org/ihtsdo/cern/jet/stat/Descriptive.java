...
*/
package org.ihtsdo.cern.jet.stat;
 ...
 * <br><tt>b(k,i) = </tt>{@link org.ihtsdo.cern.jet.math.Arithmetic#binomial(long,long) binomial(k,i)} and
...
public static double moment(int k, double c, int size, double[] sumOfPowers) {
	...
	for (int i=0; i<=k; i++) {
		...
		//sum += sign * 
		sum += sign * org.ihtsdo.cern.jet.math.Arithmetic.binomial(k,i) * y * sumOfPowers[k-i];
		sign = -sign;
	}
	/*
	for (int i=0; i<=k; i++) {
		sum += sign * org.ihtsdo.cern.jet.math.Arithmetic.binomial(k,i) * Math.pow(c, i) * sumOfPowers[k-i];
		sign = -sign;
	}
	*/
	return sum/size;
