/*
Copyright &copy;py; 1999 CERN - European Organization for Nuclear Research.
...
*/
package org.ihtsdo.cern.jet.math;

import org.ihtsdo.cern.colt.function.DoubleDoubleFunction;
import org.ihtsdo.cern.colt.function.DoubleDoubleProcedure;
import org.ihtsdo.cern.colt.function.DoubleFunction;
...
<p>Unary functions are of type {@link org.ihtsdo.cern.colt.function.DoubleFunction}, binary functions 
  of type {@link org.ihtsdo.cern.colt.function.DoubleDoubleFunction}. All can be retrieved via <tt>public 
  static final</tt> variables named after the function. 
Unary predicates are of type {@link org.ihtsdo.cern.colt.function.DoubleProcedure}, binary predicates 
  of type {@link org.ihtsdo.cern.colt.function.DoubleDoubleProcedure}. All can be retrieved via <tt>public 
...
public static void demo1() {
	org.ihtsdo.cern.jet.math.Functions F = org.ihtsdo.cern.jet.math.Functions.functions;
	double a = 0.5; 
	double b = 0.2;
	double v = Math.sin(a) + Math.pow(Math.cos(b),2);
	System.out.println(v);
...
public static void demo2(int size) {
	...
	// emptyLoop
	org.ihtsdo.cern.colt.Timer emptyLoop = new org.ihtsdo.cern.colt.Timer().start();
	a = 0; b = 0;
	double sum = 0;
	for (int i=size; --i >= 0; ) {
		sum += a;
	...
	org.ihtsdo.cern.colt.Timer timer = new org.ihtsdo.cern.colt.Timer().start();
	a = 0; b = 0;
	sum = 0;
	for (int i=size; --i >= 0; ) {
 ...
 * Currently the engine is {@link org.ihtsdo.cern.jet.random.engine.MersenneTwister}
 * and is seeded with the current time.
 * <p>
 * Note that any random engine derived from {@link org.ihtsdo.cern.jet.random.engine.RandomEngine} and any random distribution derived from {@link org.ihtsdo.cern.jet.random.AbstractDistribution} are function objects, because they implement the proper interfaces.
...
public static DoubleFunction random() {
	return new org.ihtsdo.cern.jet.random.engine.MersenneTwister(new java.util.Date());
}
/**
