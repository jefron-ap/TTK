...
*/
public abstract class AbstractIntList extends AbstractList implements org.ihtsdo.cern.colt.buffer.IntBufferConsumer {
	/**
...
public void mergeSortFromTo(int from, int to, IntComparator c) {
	...
	int[] myElements = elements();
	org.ihtsdo.cern.colt.Sorting.mergeSort(myElements, from, to+1, c);
	elements(myElements);
	setSizeRaw(mySize);
}
/**
...
public void quickSortFromTo(int from, int to, IntComparator c) {
	...
	int[] myElements = elements();
	org.ihtsdo.cern.colt.Sorting.quickSort(myElements, from, to+1,c);
	elements(myElements);
	setSizeRaw(mySize);
}
/**
...
public void shuffleFromTo(int from, int to) {
	...
	org.ihtsdo.cern.jet.random.Uniform gen = new org.ihtsdo.cern.jet.random.Uniform(new org.ihtsdo.cern.jet.random.engine.DRand(new java.util.Date()));
	for (int i=from; i<to; i++) { 
