...
import org.ihtsdo.cern.colt.function.CharComparator;
import org.ihtsdo.cern.colt.function.DoubleComparator;
import org.ihtsdo.cern.colt.function.FloatComparator;
import org.ihtsdo.cern.colt.function.IntComparator;
import org.ihtsdo.cern.colt.function.LongComparator;
import org.ihtsdo.cern.colt.function.ShortComparator;
/**
 ...
 * @see org.ihtsdo.cern.colt.matrix.doublealgo.Sorting
...
if (numNegZeros != 0) {
	int j = new org.ihtsdo.cern.colt.list.DoubleArrayList(a).binarySearchFromTo(0.0d, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0d);
...
// Postprocessing phase: change 0.0's to -0.0's as required
if (numNegZeros != 0) {
	int j = new org.ihtsdo.cern.colt.list.FloatArrayList(a).binarySearchFromTo(0.0f, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0f);
